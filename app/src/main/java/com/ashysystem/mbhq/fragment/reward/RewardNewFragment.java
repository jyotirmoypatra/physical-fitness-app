...
public class RewardNewFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        lblGratitudeStreak.setText("Gratitude Streak");
    ...
    private void populateData() {
        if (!sharedPreference.read("CURRENT_STREAK", "").equals("")) {
            txtGratitudeStreak.setText(sharedPreference.read("CURRENT_STREAK", ""));
        } else {
            txtGratitudeStreak.setText("0");
        }
        if (!sharedPreference.read("TOP_STREAK", "").equals("")) {
            txtPBStreak.setText(sharedPreference.read("TOP_STREAK", ""));
        } else {
            txtPBStreak.setText("0");
        }
        if (!sharedPreference.read("TOTAL_STREAK", "").equals("")) {
            txtTotal.setText(sharedPreference.read("TOTAL_STREAK", ""));
        } else {
            txtTotal.setText("0");
        }

        try {
            globalTotalStrakValue = Integer.parseInt(sharedPreference.read("TOTAL_STREAK", ""));
